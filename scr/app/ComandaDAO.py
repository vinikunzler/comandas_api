...
async def get_comanda(id_comanda: int):
    try:
        session = db.Session()
        aux_dados = (
            session.query(ComandaDB, FuncionarioDB, ClienteDB)
...
async def get_comanda_by_status(status: int):
    try:
        session = db.Session()
...
async def post_comanda_item(corpo: ComandaProdutos):
    try:
        ...
        dados = ComandaProdutoDB(
            None, corpo.comanda_id, corpo.produto_id, corpo.funcionario_id, corpo.quantidade, corpo.valor_unitario
        )
        session.add(dados)
        # session.flush()
        session.commit()
...
@router.get("/comandas/{comanda_id}/total", tags=["Comanda"])
